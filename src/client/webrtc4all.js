...
function WebRtc4all_GuessType() {
    // WebRtc plugin type
    try {
        var nativeRTCPeerConnection = (window.webkitPeerConnection00 || window.webkitRTCPeerConnection || window.mozRTCPeerConnection);
        var nativeRTCSessionDescription = (window.mozRTCSessionDescription || window.RTCSessionDescription); // order is very important: "RTCSessionDescription" defined in Nighly but useless
        var nativeRTCIceCandidate = (window.mozRTCIceCandidate || window.RTCIceCandidate);
        var nativeURL = (window.webkitURL || window.URL);
        var nativeGetUserMedia = (navigator.webkitGetUserMedia || navigator.mozGetUserMedia);
        if (nativeGetUserMedia && nativeRTCPeerConnection) {
            return WebRtcType_e.NATIVE; // Google Chrome
        }
        else if (nativeGetUserMedia && window.webkitPeerConnection) {
            return WebRtcType_e.ERICSSON;
        }
    }
    catch (e) { }
    try {
        if ((__b_webrtc4ie_peerconn = new ActiveXObject("webrtc4ie.PeerConnection"))) {
            return WebRtcType_e.IE; // Internet Explorer
        }
    }
    catch (e) {
        var oWebRtc4npapi = document.createElement('embed');
        if (oWebRtc4npapi.supportsPeerConnection) {
            return WebRtcType_e.NPAPI; // Opera, Firefox or Safari
        }
    }
    return WebRtcType_e.NONE;
}
    ...
    if (__looper == undefined) {
        try {
            ...
            if (!(__looper = oLooper.hWnd)) {
                console.error("Failed to create looper");
            }
        }
        catch (e) {
            console.error(e);
...
function WebRtc4all_SetLocalDisplay(o_local_elt, o_local_id) {
    if (__webrtc_type == WebRtcType_e.IE) {
        // visiblity must be "visible"  for the first time to force handle creation
        if (o_local_elt) {
            o_local_elt.innerHTML = "<object id=\"" + o_local_id + "\" classid=\"clsid:5C2C407B-09D9-449B-BB83-C39B7802A684\"" +
            ...
            __o_display_local = document.getElementById(o_local_id);
            __o_display_local.style.visibility = "hidden";
        }
    }
    else if (__webrtc_type == WebRtcType_e.NPAPI) {
        if (o_local_elt) {
            o_local_elt.innerHTML = "<embed id=\"" + o_local_id + "\" type=\"application/w4a-display\"" +
...
function WebRtc4all_AddRemoteDisplay(o_pc, o_local_elt) {
    if (pc) {
        __o_display_remote = o_local_elt;
        try { pc.o_peer.remoteVideo = (__o_display_remote ? __o_display_remote.hWnd : 0); } catch (e) { }
    }
}
...
w4aPeerConnection.prototype.processIceMessage = function (o_candidate) {
    console.error("Not implemented"); // we expect all ICE candidates in the SDP offer (SIP)
}
...
w4aPeerConnection.prototype.onIceCallback = function (media, label, bMoreToFollow) {
    console.info("onIceCallback(" + media + "," + label + "," + bMoreToFollow + ")");
